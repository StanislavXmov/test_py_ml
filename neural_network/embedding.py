...
class WordsDataset(data.Dataset):
    def __init__(self, path, navec_emb, prev_words=3):
        ...
        with open(path, "r", encoding="utf-8") as f:
            ...
            self.text = re.sub(
                r"[^А-яA-z- ]", "", self.text
            )  # удаляем все неразрешенные символы

        self.words = self.text.lower().split()
        self.words = [
            word for word in self.words if word in self.navec_emb
    ...
    def __getitem__(self, item):
        _data = torch.vstack(
            [
                torch.tensor(self.navec_emb[self.words[x]])
...
if __name__ == "__main__":
    ...
    d_train = WordsDataset("text", navec, prev_words=3)
    ...
    st = model.state_dict()
    torch.save(st, "model_rnn_words2.tar")

    model.eval()
    predict = "смотреть в будущее".lower().split()
    total = 10
